...
var last_obstacle_postion : String
...
func _on_spawner_timer_timeout() -> void:
	...
	if random == 0:
		...
		last_obstacle_postion = "up"
		
	if random == 1:
		obstacle_instance.position.y = 800
		obstacle_instance.scale.y *= -1
		last_obstacle_postion = "down"

func _on_coin_timer_timeout() -> void:
	var random_position : int = randi() % 3
	
	if random_position == 0 and last_obstacle_postion == 'up':
		return
	if random_position == 2 and last_obstacle_postion == "down":
		return
